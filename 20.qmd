...
**VAE models** have an elegant structure, but they normally operate in the latent space of lower dimensionality, which can limit their ability to capture the complexity of the data distribution. Ever since the advent of residual networks, which helps to significantly smooth the error landscape of deep networks, very deep neural networks can be effectively trained, so the idea has now been incorporated into almost all modern generative models. Besides, residual networks also **require** the data dimensionality kept constant throughout the networks. These two factors lead to **diffusion models**, which can be viewed as hierarchical VAEs, with multiple VAE models being stacked on top of each other, and the latent space dimensionality of each VAE now being the same as the data dimensionality.

The **forward encoder** in diffusion models defines a series of conditional distributions, which, starting from the observed data, gradually adds noise to the data through a predefined schedule till the data is completely indistinguishable from Gaussian noise. The process is "predefined": we define it when building the model and it's not affected by the subsequent network training. (Although extensions to the basic model often make some aspects of the forward process also trainable.)

Because directly calculating the reverse conditional distribution is infeasible (unlike **Normalizing Flows**), we train a neural network to approximate it, and this is called the **reverse decoder**. This network, starting from pure noise, attempts to remove the noise added in the forward process, and thus generates new data. 

When training diffusion models, our training objective is generally to maximize the data likelihood. However, there can be multiple ways to go about this. The first approach, by carefully decomposing and lower bounding the data likelihood, reduces the training objective to predicting the noise added to the data at each step.

Another approach is **score matching**, which makes use of the fact that the data likelihood is both a function of the network parameters, and the input data, and considers how we can wiggle the data to maximize the likelihood. This approach trains a network to match the gradients of the log data likelihood (the so-called score). We of course don't know the data likelihood, even less its gradients, so a fair amount of approximations are (again) in order.
...
&= \mathbb{E}_{q} \left[ \ln p(\mathbf{z}_T) + \sum_{t=2}^{T} \ln \frac{p(\mathbf{z}_{t-1}|\mathbf{z}_t, \mathbf{w})}{q(\mathbf{z}_{t}|\mathbf{z}_{t-1}, \mathbf{x})} - \ln q(\mathbf{z}_1|\mathbf{x}) + \ln p(\mathbf{x}|\mathbf{z}_1, \mathbf{w}) \right]
\end{aligned}
$$
...
Note that we can choose **ANY** approximate posterior function (first line, just like the ELBO for VAEs), but the one we **ACTUALLY** choose is the forward encoder distribution (second line). This is no coincidence, we introduced the forward encoder, exactly for the purpose that it can be used here. And this is exactly how we expanded VAE models to diffusion models.

The first term of the ELBO is constant w.r.t. the model parameters and is thus ignored.

For the second term, we can first sample z~t-1~ using the forward process, then sample z~t~ using the transition kernel, then we will be able to compute p and q with no problem. This is procedurally right, however the estimation will be very noisy, since we consecutively did two sampling steps.

The third term is constant w.r.t. the model parameters and is thus ignored.

For the fourth term we can first sample z~1~ and then calculate the value.
...
Using this, we can rewrite the ELBO as 

$$
\begin{aligned}
\mathcal{L}(\mathbf{w}) = \mathbb{E}_{q} \left[ \sum_{t=2}^{T} \ln \frac{p(\mathbf{z}_{t-1}|\mathbf{z}_t, \mathbf{w})}{q(\mathbf{z}_{t-1}|\mathbf{z}_{t}, \mathbf{x})} + \ln p(\mathbf{x}|\mathbf{z}_1, \mathbf{w}) \right]
\end{aligned}
$$
...
1. sample x, t, e
2. forward diffusion to compute z
3. forward pass to compute g, L
4. backward pass to update w

/the forward encoder is precomputed./

### generating new samples

![](./img/DLFC/Chapter-20/algo20.2.png)

the sampling has to been done step by step.

1. draw last z
2. forward pass to compute g
3. denoise to compute previous mu
4. add noise to get previous z
...
The training matches the model score to the data distribution score.

$$
J(\mathbf{w}) = \frac{1}{2} \int \left\| \mathbf{s}(\mathbf{x}, \mathbf{w}) - \nabla_{\mathbf{x}} \ln p(\mathbf{x}) \right\|^2 p(\mathbf{x}) \, d\mathbf{x}
$$

Of course this is only theoretical: we don't actually know the data distribution. Next we move on to workarounds.

### modified score loss

We first work on approximating the data distribution. We approximate it with the empirical one
$$
p_{\mathcal{D}}(\mathbf{x}) = \frac{1}{N} \sum_{n=1}^{N} \delta(\mathbf{x} - \mathbf{x}_n).
$$

This essentially is a uniform distribution over the data points. Since it is not differentiable, we then smooth it with the kernel density estimator 
$$
q_{\sigma}(\mathbf{z}) = \int q(\mathbf{z} | \mathbf{x}, \sigma)p(\mathbf{x}) \, d\mathbf{x}
$$

where $q(\mathbf{z} | \mathbf{x}, \sigma)$ is a Gaussian distribution centered at $\mathbf{x}$ with standard deviation $\sigma$.

Now we have a known, differentiable distribution of the newly introduced variable $\mathbf{z}$. We can then proceed to define the score matching loss function

$$
\begin{align}
J(\mathbf{w}) &= \frac{1}{2} \int \left\| s(\mathbf{z}, \mathbf{w}) - \nabla_{\mathbf{z}} \ln q_\sigma(\mathbf{z}) \right\|^2 q_\sigma(\mathbf{z}) \mathrm{d}\mathbf{z}, \\
&= \frac{1}{2} \int \int \left\| s(\mathbf{z}, \mathbf{w}) - \nabla_{\mathbf{z}} \ln q(\mathbf{z} | \mathbf{x}, \sigma) \right\|^2 q(\mathbf{z} | \mathbf{x}, \sigma) p(\mathbf{x}) \mathrm{d}\mathbf{z} \mathrm{d}\mathbf{x} + \text{const.}, \\
&= \frac{1}{2N} \sum_{n=1}^{N} \int \left\| s(\mathbf{z}, \mathbf{w}) - \nabla_{\mathbf{z}} \ln q(\mathbf{z} | \mathbf{x}_n, \sigma) \right\|^2 q(\mathbf{z} | \mathbf{x}_n, \sigma) \mathrm{d}\mathbf{z} + \text{const.}
\end{align}
$$

1. we are matching scores of the latents, not the data.
2. By using the approximate data distribution, which integrates out the data, we are working with a double integral.
3. Since the data distribution is essentially uniform on point sets, we can simply sum over the data points.

In the end, note we are matching scores of the latents, not the data. We have arrived, at this point, to how to choose the noise level $\sigma$ for the kernel density estimator. This problem is comparable to choosing the forward diffusion process in the previous section.
...
To generate a desired image, we can collect a dataset with similar images, learn its distribution, and then sample from it; or we can collect a dataset with many different images, learn a distribution with different modes, and then, when we want to generate an image with certain traits, we can somehow guide the sampling process to find the mode that corresponds to these desired traits. Obviously the second approach is more flexible and powerful, and that's what we'd expect from a good fundation model. It also turns out that this guided diffusion process can be used for many other purposes, such as image inpainting, outpainting, and super-resolution.
